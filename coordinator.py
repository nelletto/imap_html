...
class ImapMessage:
    ...
    def __init__(self, force_html: bool, raw_message: bytes) -> None:
    ...
    def text(self) -> str:
        ...
        for part in self.email_message.walk():
            if self.force_html:
                message_html = str(
                    self._decode_payload(self.email_message.get_payload()[1])
                )
            elif part.get_content_type() == CONTENT_TYPE_TEXT_PLAIN:
...
class ImapDataUpdateCoordinator(DataUpdateCoordinator[int | None]):
    ...
    def __init__(
        ...
        self._force_html = entry.data.get(CONF_FORCE_HTML)
    ...
    async def _async_process_event(self, last_message_uid: str) -> None:
        ...
        if response.result == "OK":
            message = ImapMessage(self._force_html, response.lines[1])
